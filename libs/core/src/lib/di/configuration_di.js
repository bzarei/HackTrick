...
import { AnnotationResolver, injectable } from "./di";
export class ConfigValue {
    key;
    default_;
    constructor(key, default_ = undefined) {
        this.key = key;
...
export function config(key, defaultValue) {
    return (target, propertyKey, parameterIndex) => {
        const clazz = propertyKey == null ? target : target.constructor;
        const metaKey = propertyKey == null ? `param:${parameterIndex}` : `${String(propertyKey)}:param:${parameterIndex}`;
        const existing = Reflect.getMetadata('annotations', clazz, metaKey) || [];
        existing.unshift(new ConfigValue(key, defaultValue));
        Reflect.defineMetadata('annotations', existing, clazz, metaKey);
    };
}
let ConfigAnnotationResolver = class ConfigAnnotationResolver extends AnnotationResolver {
    // constructor
    constructor() {
        super(ConfigValue);
    }
    ...
    resolve(annotationValue, paramType, environment, ...deps) {
...
ConfigAnnotationResolver = __decorate([
    injectable({ module: "boot" }),
    __metadata("design:paramtypes", [])
], ConfigAnnotationResolver);
export { ConfigAnnotationResolver };
//# sourceMappingURL=configuration_di.js.map